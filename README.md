...
The GUI workspace card is capped at four visible sidebar lines: one brief generated title, one plain-text count row, cmux's native OMP lifecycle row, and aggregate task progress. The count row always includes all five root-plus-subagent states as `Run`, `Block`, `Done`, `Ask`, and `Err`, including zero counts. It omits agent names, per-state rows, Markdown formatting, and `Show more`, so card height never grows with the number of agents.

For each root prompt, the plugin asks `openai-codex/gpt-5.6-luna` through OMP's configured model registry for one plain-text title, strips multiline or Markdown noise, and hard-limits the result to 32 Unicode characters. Model or authentication failures retain a bounded deterministic lifecycle title. Workspace progress combines root Todo items with subagent units and reports compact aggregate completion, running-agent count, and errors.

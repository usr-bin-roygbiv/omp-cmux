...
GUI cmux status follows OMP through `idle`, `working`, `thinking`, `tool`, `needs-input`, `retrying`, `compacting`, `waiting`, `done`, `error`, and `stopped`. The native `cmux hooks omp session-start|prompt-submit|stop` bridge keeps cmux's built-in OMP lifecycle current for the exact captured workspace and surface. Tool status includes the active tool name. Ask gates publish `Needs input`, flash the originating surface, and only then send the decision notification; resolving the matching Ask restores the derived lifecycle status.

The surface-scoped workspace card uses a bounded title plus five deterministic agent groups: `Running`, `Blocked`, `Completed`, `Needs decision`, and `Errored`. Every group is always published, including zero-count groups, with exact root-and-subagent counts, bounded agent names, Markdown status format, and a fixed blue/amber/green/purple/red palette. cmux collapses metadata after three entries, so the compact card shows `Show more` for the remaining fixed rows instead of growing with the number of agents.

For each root prompt, the plugin asks `openai-codex/gpt-5.6-luna` through OMP's configured model registry for one plain-text title, strips multiline or Markdown noise, and hard-limits the result to 64 Unicode characters. Model or authentication failures retain the deterministic lifecycle title. Workspace progress combines root Todo items with subagent units and reports aggregate completion, running-agent count, and errors.
...
When a Linux cmux TUI is viewed through SSH, the plugin keeps that native `cmux-tui notify` delivery and also writes one bounded OSC 777 notification to each live matching SSH attach viewer. Matching uses the active TUI socket or session; live viewers take precedence over an inherited `SSH_TTY`, so a detached session does not target a stale terminal. The relay strips control characters, sends only the fixed semantic outcome plus sanitized machine/interface context, deduplicates shared pseudo-terminals, and fails closed without a safe `/dev/pts/<id>` route. It adds no watcher, network service, or persisted bridge state.
